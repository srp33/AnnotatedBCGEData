...
The `getDataset` function accepts a vector of dataset identifiers. At the time of package development, the Zenodo API limits the number of requests per minute to 30. Because of this, we found it necessary to pause execution between successive calls to Zenodo. These API limits should not cause problems for the user, but users should be aware that this may cause delays when downloading large amounts of data.
